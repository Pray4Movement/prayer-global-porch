 ...
 *
 */
abstract class Prayer_Global_Migration {
    ...
    /**
     * @throws \Prayer_Global_Migration_Test_Exception Table $name not as expected, see error log.
     */
    protected function test_expected_tables() {
        ...
        foreach ( $expected_tables as $name => $expected_table ) {
            $got_table = $wpdb->get_var( "SHOW CREATE TABLE `$name`", 1, 0 ); //phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
            ...
            if ( $got_table !== $expected_table ) {
                ...
                //throw new Prayer_Global_Migration_Test_Exception( "Table $name not as expected, see error log" );
            }
        }
    }
...
/**
 * Class Prayer_Global_Migration_Test_Exception
 */
class Prayer_Global_Migration_Test_Exception extends Exception {
}
