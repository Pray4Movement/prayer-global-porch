...
function PGDisplayTime(time) {
  ...
  if ( time.hasOwnProperty('years') ) {
    timeList.push(`<span class="time-value"><span>${time.years}</span> <span class="time-label">${jsObject.translations['years']}</span></span>`)
  }
  if ( time.hasOwnProperty('days') ) {
    timeList.push(`<span class="time-value"><span>${time.days}</span> <span class="time-label">${jsObject.translations['days']}</span></span>`)
  }
  if ( time.hasOwnProperty('hours') ) {
    timeList.push(`<span class="time-value"><span>${padNumber(time.hours)}</span> <span class="time-label">${jsObject.translations['hours']}</span></span>`)
  }
  if ( time.hasOwnProperty('minutes') ) {
    timeList.push(`<span class="time-value"><span>${padNumber(time.minutes)}</span> <span class="time-label">${jsObject.translations['minutes']}</span></span>`)
  }
  if ( time.hasOwnProperty('seconds') ) {
    timeList.push(`<span class="time-value"><span>${padNumber(time.seconds)}</span> <span class="time-label">${jsObject.translations['seconds']}</span></span>`)
  }
