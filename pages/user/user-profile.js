window.addEventListener("load", function () {
  ...
  jQuery("#confirm-user-account-delete").on("click", () => {
    get_user_app("delete_user").then((confirmed) => {
      if (confirmed) {
        window.location = "/";
      }
    });
  ...
  function write_main(data) {
        ...
        <div class="flow-medium">
                ...
                <div class="user__info">
                    ...
                    <p class="user__location">
                        ...
                        <span class="iplocation-message small d-block text-secondary">
                            ${
                              data.location && data.location.source === "ip"
                                ? estimated_location
                                : ""
                            }
                        </span>
            ...
            <section>
                ...
                <div class="d-flex flex-column m-auto w-fit">
                    <a class="btn btn-small btn-primary-light uppercase" data-reverse-color href="/give" target="_blank">${give} <i class="ion-android-open"></i></a>
    ...
    Promise.resolve()
      ...
      .then(() =>
        get_user_app("link_anonymous_prayers", {
          hash: pg_user_hash,
          user_id: jsObject.user.ID,
        })
      )
      .then(({ has_updates }) => {
        if (has_updates) {
          return Promise.all([getStats(), getActivity()]);
        }
      });
    ...
    const userDetailsContentHTML = `
            ...
            <table class="table">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="user__location-label">${
                              (location && location.label) || select_a_location
                            }</span>
                            <span class="iplocation-message small d-block text-secondary">
                                ${
        ...
        <section class="communication-preferences flow-small">
            ...
            <div>
                <div class="form-check small">
                    <input class="form-check-input user-check-preferences" type="checkbox" id="send_general_emails" ${
                      send_general_emails && "checked"
                    }>
        ...
        <section class="user-actions">
            ...
            ${ModalButton({
              ...
              classes:
                "small uppercase btn btn-small btn-outline-danger d-block mt-3",
            })}
        </section>`;
  ...
  function write_prayers() {
    ...
    if (jsObject.user.activity && jsObject.user.stats) {
      ...
      const handlePrimaryContent = ({ location_name, time_prayed_text }) =>
        time_prayed_for
          .replace("%1$s", time_prayed_text)
          .replace("%2$s", location_name || location_text);
      const handleSecondaryContent = ({ time_prayed_text, group_name }) =>
        in_group_text.replace("%s", group_name);
      ...
      jQuery(".user-activity__list").html(
        PG.ActivityList(logs, handlePrimaryContent, handleSecondaryContent)
      );
      ...
      if (logsLeft > 0) {
        ...
        const getMoreActivity = () => {
          ...
          get_user_app("activity", { offset: offset + limit, limit }).then(
            (newActivity) => {
              const activity = {
                offset: newActivity.offset,
                limit: newActivity.limit,
                logs: [...jsObject.user.activity.logs, ...newActivity.logs],
              };
              jsObject.user.activity = activity;

              jQuery(".user-activity__list").append(
                PG.ActivityList(
                  newActivity.logs,
              ...
              const newLogsLeft =
                total_locations - (newActivity.offset + limit);
  ...
  function write_challenges() {
    ...
    jQuery("#challenge-form").on("submit", (event) => {
      ...
      const challengeType = jQuery('input[name="challenge-type"]:checked').attr(
        "id"
      );
  ...
  function setChallengeForm({
    ...
    jQuery('input[name="challenge-type"]#' + challenge_type).prop(
      "checked",
      true
    );
  ...
  function getStats() {
    return get_user_api("stats").then((stats) => {
  ...
  function getActivity() {
    return get_user_app("activity").then((activity) => {
  ...
  function getChallenges(visibility, callback) {
    return get_user_app("get_challenges", { visibility }).then((challenges) => {
  ...
  function buildChallengeList(visibility) {
    ...
    jQuery(containerSelector + " .edit-challenge-button").on(
      "click",
      function () {
        ...
        const challenge = challenges.find(
          ({ post_id }) => Number(post_id) === challengeId
        );
