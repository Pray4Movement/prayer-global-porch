window.addEventListener("load", function ($) {
  ...
  const detailsType = jsObject.hasOwnProperty("details_type")
    ? jsObject.details_type
    : defaultDetailsType;
  const mapType = jsObject.hasOwnProperty("map_type")
    ? jsObject.map_type
    : defaultMapType;
  ...
  const toggleParticipantsElement =
    document.getElementById(toggleParticipantsId);
  const toggleUserLocationsElement = document.getElementById(
    toggleUserLocationsId
  );
  ...
  const pray_for_area_content =
    pray_for_area_modal && pray_for_area_modal.querySelector(".modal-content");
  const pray_for_area_button = jQuery("#pray-for-area-button");
  const cta_modal = document.getElementById("cta_modal");
  const cta_modal_body = cta_modal
    ? cta_modal.querySelector(".modal-body")
    : null;
  ...
  function load_grid() {
      ...
      Promise.all(
        ...
        map.addLayer({
          ...
          layout: {
            ...
            "icon-size": ["interpolate", ["linear", 1], ["zoom"], ...iconSizes],
            "icon-padding": 0,
    ...
    map.on("load", function () {
      ...
      let iconSize = 1;
      map.loadImage(tickImage, (error, image) => {
    ...
    map.on("idle", () => {
      if (
        !map.getLayer(participantsLayerId) ||
        !map.getLayer(userLocationsLayerId)
      ) {
        return;
      }

      const toggleElements = document.querySelectorAll(
        `.map-toggle[data-layer-id], .map-toggle[data-source-id]`
      );
    ...
    if (jsObject.stats.remaining_int < 1) {
      ...
      jQuery(".needed_locations_per_hour").html(
        jsObject.stats.needed_locations_per_hour
      );
      jQuery(".needed_locations_per_day").html(
        jsObject.stats.needed_locations_per_day
      );
    ...
    const timeSinceMidnight =
      challengeDate.getSeconds() * 1000 +
      challengeDate.getMinutes() * 60 * 1000 +
      challengeDate.getHours() * 60 * 60 * 1000;
    ...
    if (Date.now() < midnightOfChallengeDate) {
      ...
      */
      ...
      jQuery(".starts-on-date").html(
        `${jsObject.stats.start_time_formatted} <br/> ${startTime}`
      );
  ...
  function formatTimeLeft(timeLeft) {
    ...
    let seconds = Math.floor(
      timeLeft - minutes * 60 - hours * 60 * 60 - days * 24 * 60 * 60
    );
  ...
  function getFillColors(mapType, layers) {
    ...
    const heatmapFill = {
      "fill-color": ["step", ["get", "value"], ...stepColors],
      "fill-opacity": 0.9,
  ...
  function show_cta() {
    ...
    if (show_cta && jsObject.is_cta_feature_on === true) {
      window.api_post_global("ctas", "get_cta").then((cta) => {
        if (!cta) {
          return;
        }
        const content = `
            <h3 class="modal-title">${cta.post_title}</h3>
            ${cta.post_content.replace("wp-element-button", "btn btn-primary")}
          `;
