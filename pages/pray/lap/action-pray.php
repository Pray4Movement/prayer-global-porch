...
class PG_Pray_Lap extends PG_Public_Page {
    public $url_path = 'pray';
    ...
    public function wp_enqueue_scripts() {
        ...
        pg_enqueue_script( 'lap-js', 'pages/pray/lap/lap.js', [ 'global-functions', 'report-js' ], true );

        pg_enqueue_style( 'lap-css', 'pages/pray/lap/lap.css' );

        require_once( Prayer_Global_Porch::get_dir_path() . 'pages/assets/header-event.php' );
        require_once( Prayer_Global_Porch::get_dir_path() . 'utilities/security.php' );

        $current_url = trailingslashit( site_url() ) . dt_get_url_path( true );

        $svg_manager = new SVG_Spritesheet_Manager();
        $spritesheet_url = $svg_manager->get_cached_spritesheet_url( $this->icons, 'pg' );

        wp_localize_script( 'lap-js', 'jsObject', [
            ...
            'translations' => [
                ...
                'you_have_earned_a_badge' => esc_html__( 'You just earned a new badge.', 'prayer-global-porch' ),
                'you_have_earned_badges' => esc_html__( 'You just earned new badges.', 'prayer-global-porch' ),
            ...
            'image_folder' => Prayer_Global_Porch::get_url_path() . 'assets/images/',
            'current_url' => $current_url,
            'map_url' => $current_url . 'map',
            'is_custom' => $this->custom_relay,
            'is_cta_feature_on' => true,
            'user_id' => get_current_user_id(),
            'cache_url' => 'https://s3.prayer.global/',
            'direct_api_url' => Prayer_Global_Porch::get_url_path(),
            'icons_url' => Prayer_Global_Porch::get_url_path() . 'assets/images/icons',
            'badges_url' => Prayer_Global_Porch::get_url_path() . 'assets/images/badges',
    ...
    public function header_javascript(){
        require_once( Prayer_Global_Porch::get_dir_path() . 'pages/assets/footer.php' );
        require_once( Prayer_Global_Porch::get_dir_path() . 'pages/assets/share-modal.php' );
    }
    ...
    public function body(): void {
        ...
        <?php echo file_get_contents( $spritesheet_dir ); //phpcs:ignore?>
                    ...
                    <div class="modal-header">
                        <button type="button" id="welcome_screen_close" data-bs-dismiss="modal" aria-label="<?php esc_attr( __( 'Close', 'prayer-global-porch' ) ) ?>">
                            <svg class="f-xlg" height="1em" width="1em" viewBox="0 0 33 33" fill="currentColor" >
                                <use href="<?php echo esc_url( Prayer_Global_Porch::get_url_path() . 'pages/assets/images/close.svg#pg-icon' ) ?>"></use>
                            </svg>
                        </button>
                    </div>
                    <div class="modal-body">
                        ...
                        <p>
                            <img src="<?php echo esc_url( Prayer_Global_Porch::get_url_path() . 'pages/assets/images/welcome-keep.png' ) ?>" style="opacity:0.5;" class="img-fluid" />
                        </p>
                        ...
                        <p>
                            <img src="<?php echo esc_url( Prayer_Global_Porch::get_url_path() . 'pages/assets/images/welcome-next.png' ) ?>" style="opacity:0.5;" class="img-fluid" />
                        </p>
                    </div>
