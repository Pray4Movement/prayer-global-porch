window.addEventListener("load", function () {
  ...
  function get_next_grid_id(relayId) {
    return window.api_fetch(`/next-location.php?relay_id=${relayId}`);
  }
  function get_location_prayer_content() {
    const url = new URL(location.href);
    const grid_id = url.searchParams.get("grid_id");
    // load current location
    let action = url.searchParams.has("all-content")
      ? "refresh-all"
      : "refresh";
    return window.api_post(action, { grid_id });
  }
  function get_next_location() {
    const relayId = jsObject.parts.public_key;
    get_next_grid_id(relayId).then(({ next_location }) => {
      return get_location_prayer_content(next_location);
    });
  }
  function load_next_content() {
    get_next_location().then(function (location) {
      if (location === false) {
        window.location =
          "/" +
          jsObject.parts.root +
