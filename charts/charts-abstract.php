...
abstract class PG_Metrics_Chart_Base
{
    ...
    public function base_menu( $content ) {
        $line = '<li><a href="'. site_url( '/review/'.$this->base_slug.'/' . $this->slug ) . '">' . $this->title . '</a></li>';

        $ref = '<ul class="menu vertical nested" id="' . $this->base_slug . '-menu">';
        $pos = strpos( $content, $ref );
        if ( $pos === false ){
            $content .= '
            <li><a href="'. site_url( '/review/'. $this->base_slug .'/'. $this->slug ) .'">'.$this->base_title.'</a>
    ...
    public function base_add_url( $template_for_url ) {
        $template_for_url["review/$this->base_slug/$this->slug"] = 'template-metrics.php';
