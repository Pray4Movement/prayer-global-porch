DROP TABLE IF EXISTS 9VJS6H_dt_relays;
CREATE TABLE 9VJS6H_dt_relays (
...
BEGIN
    ...
    DECLARE cursorActiveLaps CURSOR FOR
        SELECT ID from 9VJS6H_posts
            JOIN 9VJS6H_postmeta p ON ID = p.post_id AND p.meta_key = 'status' AND p.meta_value = 'active'
            JOIN 9VJS6H_postmeta p1 ON ID = p1.post_id AND p1.meta_key = 'type' AND p1.meta_value = lap_type;
...
BEGIN
    ...
    DECLARE cursorLaps CURSOR FOR
        WITH RECURSIVE cte ( p2p_from_, p2p_to_ ) AS (
            ...
            SELECT p2p_from, p2p_to FROM 9VJS6H_p2p WHERE p2p_to = id AND p2p_type = 'parent-lap_to_child-lap'
            UNION ALL
            ## output the next link in the chain
            SELECT _next.p2p_from, _next.p2p_to
            FROM cte _current
            JOIN 9VJS6H_p2p _next
    ...
    IF (SELECT meta_value FROM 9VJS6H_postmeta WHERE meta_key = 'global_lap_number' AND post_id = id) IS NULL THEN
        INSERT INTO 9VJS6H_postmeta (post_id, meta_key, meta_value)
        VALUES ( id, 'global_lap_number', lap_number + 1 );
    ELSE
        UPDATE 9VJS6H_postmeta
...
BEGIN
    ...
    INSERT INTO 9VJS6H_dt_relays ( relay_id, grid_id, total )
    SELECT relay_key as relay_id, r.grid_id, lap_number as total FROM 9VJS6H_dt_reports r
    JOIN 9VJS6H_postmeta pm ON r.post_id = pm.post_id
    WHERE pm.meta_key = 'prayer_app_relay_key'
    AND pm.meta_value = '49ba4c'
    GROUP BY r.grid_id;

    IF relay_key = '49ba4c' THEN
        UPDATE 9VJS6H_dt_relays l
        JOIN (
            SELECT relay_key as relay_id, r.grid_id, lap_number as total FROM 9VJS6H_dt_reports r
